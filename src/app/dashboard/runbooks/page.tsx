...
import { 
  ...
  MoreVertical, X, Check, Edit3, Palette, Share2, Mail, 
...
export default function RunbooksPage() {
  ...
  // Sharing state
  const [sharingRunbook, setSharingRunbook] = useState<Runbook | null>(null);
  ...
  // Open share modal
  const openShareModal = async (runbook: Runbook) => {
    setSharingRunbook(runbook);
    setShareEmail('');
  ...
  const createShare = async () => {
    if (!sharingRunbook || !shareEmail.trim()) return;
  ...
  // Copy share link
  const copyShareLink = (runbook: Runbook) => {
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-800">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => openShareModal(runbook)}
                        className="p-1.5 text-slate-500 hover:text-white hover:bg-slate-800 rounded transition-colors"
                        title="Share"
                      >
                        <Share2 size={16} />
      ...
      <AnimatePresence>
        {sharingRunbook && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4"
            onClick={() => setSharingRunbook(null)}
          >
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 border-b border-slate-800">
                <div>
                  ...
                  <p className="text-sm text-slate-400 truncate">{sharingRunbook.title}</p>
                </div>
                <button
                  onClick={() => setSharingRunbook(null)}
                  className="p-1 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors"
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="p-3 bg-slate-800 rounded-lg">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => copyShareLink(sharingRunbook)}
              ...
              <div className="flex items-center justify-between p-4 border-t border-slate-800">
                <Link
                  href={`/dashboard/runbooks/${sharingRunbook.id}`}
